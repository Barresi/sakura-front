...
const TitleGradient: FC<ITitleGradientProps> = ({ className, children }) => {
  return (
    <div className={`relative w-fit m-auto p-1 ${className}`}>
      ...
      <div
        className='absolute bottom-0 top-0 right-[-20px] rounded-[10px] max-w-[180px] w-full p-px bg-gradient-to-tl from-redHover/50 m-auto
      from-0% to-50%'
      >
        <div className='bg-white dark:bg-grayBlue rounded-[9px] h-full w-full' />
      </div>
    </div>
  )
}
